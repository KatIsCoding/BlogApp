...
RSpec.describe 'Testing Posts Index Page', type: :feature do
  before(:each) do
    ...
    Post.create(author: first_user, title: 'Two', text: 'This is my second post', commentscounter: 0,
                likescounter: 0)
  ...
  scenario 'When clicking on a post, the page redirects to that post page' do
    sleep(5)
    click_link 'One'
    expect(page.current_path).to eq "/users/#{@first_user.id}/posts/#{@first_post.id}"
  end
end
